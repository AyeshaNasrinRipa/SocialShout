...
require_once('./mailer.php');
	...
	$otp = rand(1500,9999);
		$insert = "insert into users (f_name,l_name,user_name,describe_user,Relationship,user_pass,user_email,user_country,user_gender,user_birthday,user_image,user_cover,user_reg_date,status,posts,recovery_account,OTP)
		values('$first_name','$last_name','$username','Hi there, Welcome to my profile','...','$pass','$email','$country','$gender','$birthday','$profile_pic','default_cover.jpg',NOW(),'$status','$posts','Security Question','$otp')";
		
		$query = mysqli_query($con, $insert);

		if($query){
		sendMail($email,$otp,"http://localhost/social_network/verifyaccount.php");
